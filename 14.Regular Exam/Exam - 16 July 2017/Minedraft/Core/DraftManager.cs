...
public class DraftManager
{
    ...
    public string RegisterProvider(List<string> arguments)
    {
        ...
        try
        {
            this.providers.Add(this.providerFactory.CreateProvider(type, id, energy));
        }
        catch (ArgumentException ae)
        {
            return ae.Message;
    ...
    public string Day()
    {
        var energy = this.providers.Sum(p => p.EnergyOutput);
        var neededEnergy = this.harvesters.Sum(h => h.EnergyRequirement);
        var ores = this.harvesters.Sum(h => h.OreOutput);
